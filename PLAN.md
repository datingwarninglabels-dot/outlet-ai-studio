...
- **Public marketing landing page** *(done, not deployed)*: built the
  public `/` marketing page (previously a pure redirect) per a detailed
  design/copy/engineering brief — warm amber/copper brand tokens scoped
  under `[data-brand="marketing"]` so the authenticated app's palette is
  untouched but can adopt the same semantic token names later; a
  config-driven CTA system (`lib/site-config.ts`'s `CTA_MODE`) currently
  set to `"waitlist"`; a central `lib/plans.ts` pricing config with no
  invented dollar figures; a real waitlist signup flow (honeypot + timing
  check + DB-backed rate limiting + duplicate-safe insert); SEO metadata,
  `robots.ts`, and `sitemap.ts` with private app routes excluded from
  indexing; and an open-redirect-safe login callback (`safe-redirect.ts`).
  Copy was fact-checked against the actual codebase rather than assumed —
  caught and corrected two inaccuracies before commit: an FAQ answer
  implying customers would see the Owner-only Provider Hub admin screen,
  and a "Most popular" pricing badge implying real popularity data that
  doesn't exist pre-launch (changed to "Recommended").

  **Migration**: `drizzle/0015_typical_bulldozer.sql` adds one new table,
  `waitlist_signup` — no changes to any existing table.
  - **Columns**: `id` (uuid, primary key, default `gen_random_uuid()`),
    `email` (text, not null, `UNIQUE` constraint), `creator_type` (text,
    nullable), `ip_hash` (text, nullable — a salted SHA-256 hash used only
    for rate-limiting, never the raw IP), `created_at` (timestamp, not
    null, default `now()`).
  - **Indexes/constraints**: the `email` `UNIQUE` constraint (Postgres
    creates its backing index automatically); no other indexes.
  - **To apply**: `npm run db:migrate` (runs `drizzle-kit migrate` against
    `DATABASE_URL`). The migration was generated (`npm run db:generate`)
    and reviewed but **has not been applied to any database** — no
    reachable database existed in the environment this was built in.
  - **Required env vars**: `DATABASE_URL` only, to apply it.
  - **Rollback**: no down-migration is generated by drizzle-kit's default
    workflow. Because this table is new and has no foreign keys to or
    from any existing table, rollback is a single safe statement:
    `DROP TABLE waitlist_signup;` — this cannot affect any other table's
    data or constraints.
  - **Verifying success**: after applying, `select * from waitlist_signup
    limit 1;` should return zero rows without error (confirms the table
    exists with the right shape), or inspect it via `npm run db:studio`.
  - **Testing status**: waitlist input validation (email format, consent
    requirement, honeypot, creator-type enum) has automated unit test
    coverage (`lib/validation.test.ts`). Email normalization
    (lowercasing), duplicate-signup handling (`onConflictDoNothing`),
    rate-limiting behavior, and the DB-failure path are implemented and
    code-reviewed but have **no automated test against a live database**
    (same "no DB reachable in this environment" constraint as Milestone
